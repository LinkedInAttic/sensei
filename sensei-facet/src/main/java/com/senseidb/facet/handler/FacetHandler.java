...
public abstract class FacetHandler<D> {
  ...
  public Filter buildFilter(FacetSelection sel) throws IOException {
    ...
    if (selections != null && selections.size() > 0) {
      if (sel.getSelectionOperation() == FacetSelection.ValueOperation.ValueOperationAnd) {
        filter = buildRandomAccessAndFilter(selections);
        if (filter == null) {
          filter = EmptyFilter.getInstance();
        }
      } else {
        filter = buildRandomAccessOrFilter(selections, false);
        if (filter == null) {
    ...
    if (notSelections != null && notSelections.size() > 0) {
      Filter notFilter = buildRandomAccessOrFilter(notSelections, true);
      if (filter == null) {
        filter = notFilter;
      } else {
  ...
  abstract public Filter buildRandomAccessFilter(String value) throws IOException;

  public Filter buildRandomAccessAndFilter(List<String> vals) throws IOException {
    ArrayList<Filter> filterList = new ArrayList<Filter>(vals.size());

    for (String val : vals) {
      Filter f = buildRandomAccessFilter(val);
      if (f != null) {
        filterList.add(f);
      } else {
        return EmptyFilter.getInstance();
  ...
  public Filter buildRandomAccessOrFilter(List<String> vals, boolean isNot) throws IOException {
    ArrayList<Filter> filterList = new ArrayList<Filter>(vals.size());
   ...
   */
  public FacetCountCollectorSource getFacetCountCollectorSource(FacetSelection sel,
                                                                FacetSpec ospec,
                                                                boolean groupMode) {
